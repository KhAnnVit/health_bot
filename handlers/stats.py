...
async def cmd_weightchart(callback_query: CallbackQuery):
    ...
    chart_buffer = await charts.generate_weight_chart_bytes(callback_query.from_user.id)

    if not chart_buffer:
        await callback_query.message.answer(
            "📭 Нет данных для графика", reply_markup=kb.get_skip_inline_keyboard()
        )
        return
...
async def cmd_stats(message: types.Message):
    ...
    if pressure_stats and pressure_stats.get("total_records", 0) > 0:
        ...
        text += f"  • Верхнее: {
            pressure_stats['min_systolic']}-{
            pressure_stats['max_systolic']}\n"
        text += f"  • Нижнее: {
            pressure_stats['min_diastolic']} - {
            pressure_stats['max_diastolic']}\n"
    else:
...
async def cmd_download_weight_report(callback_query: CallbackQuery):
    ...
    report_buffer = await report.generate_weight_report(callback_query.from_user.id)
...
async def cmd_download_pressure_report(callback_query: CallbackQuery):
    ...
    report_buffer = await report.generate_pressure_report(callback_query.from_user.id)
